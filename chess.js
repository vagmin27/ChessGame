...
//Coloring of board during individual's turn for each individual token.
function coloring() {
...
document.querySelectorAll('.box').forEach(item => {
    item.addEventListener('click', function () {
        ...
        // We show the available paths in green color and it's colour location through pink
        ...
        // Toggling the turn
        // Oppisite person get's the turn.
        if (tog % 2 !== 0) {
...
// function to detect check
function isKingInCheck(color) {
    const kingId = [...document.querySelectorAll('.box')].find(b => b.innerText === color + 'king')?.id;
    ...
    for (let box of boxes) {
        ...
        if (piece.startsWith(opponentColor)) {
            if (canPieceAttack(box.id, kingId, piece)) {
                return true;
            }
        }
    }
...
// Check if a piece can attack another position
function canPieceAttack(fromId, toId, piece) {
    // This function should use your movement logic to determine valid attack
    // Here we simulate a basic idea. Replace it with real logic.

    // Example for pawn (simplified):
    const [fromRow, fromCol] = getPosition(fromId);
    const [toRow, toCol] = getPosition(toId);

    if (piece === 'Wpawn') return toRow === fromRow - 1 && Math.abs(toCol - fromCol) === 1;
    if (piece === 'Bpawn') return toRow === fromRow + 1 && Math.abs(toCol - fromCol) === 1;

    // Add full logic for rook, knight, bishop, etc.
    return false;
}

function getPosition(id) {
    const raw = id.slice(1);
    return [parseInt(raw[0]), parseInt(raw[1])]; // Assuming ids like "b12"
}

// Simulate all moves and check if any removes the check
function hasLegalMovesToEscapeCheck(color) {
    const allBoxes = [...document.querySelectorAll('.box')];
    const playerPieces = allBoxes.filter(box => box.innerText.startsWith(color));

    for (let box of playerPieces) {
        const originalId = box.id;
        const piece = box.innerText;

        // Generate all potential destinations for the piece
        const destinations = getAllValidDestinations(originalId, piece);

        for (let dest of destinations) {
            const backupFrom = document.getElementById(originalId).innerText;
            const backupTo = document.getElementById(dest).innerText;

            // Try the move
            document.getElementById(originalId).innerText = '';
            document.getElementById(dest).innerText = piece;

            const inCheck = isKingInCheck(color);

            // Undo move
            document.getElementById(originalId).innerText = backupFrom;
            document.getElementById(dest).innerText = backupTo;
    ...
    return false; // No moves to escape check
}

// Integrate after each move
setTimeout(() => {
    const nextTurnColor = tog % 2 === 0 ? 'W' : 'B';
    if (isKingInCheck(nextTurnColor)) {
        if (!hasLegalMovesToEscapeCheck(nextTurnColor)) {
            alert((nextTurnColor === 'W' ? 'Black' : 'White') + ' Wins by Checkmate!');
            location.reload();
        } else {
            alert(nextTurnColor + ' is in Check!');
        }
    }
}, 100);
